...
def _validate_request_shape(module):
    ...
    vif_uuid = module.params["vif_uuid"]
    subresource = module.params["subresource"]

    if subresource and not vif_uuid:
        module.fail_json(msg="subresource requires vif_uuid")
    ...
    allowed = allowed_request_parameters(VIF_SUBRESOURCES, vif_uuid, subresource)

    if not vif_uuid:
        allowed = STANDARD_COLLECTION_PARAMS
        label = "vif collection request"
    elif not subresource:
        allowed = set()
        label = "vif detail request"
    else:
        allowed = VIF_SUBRESOURCES[subresource]["supported_params"]
        label = f"vif subresource '{subresource}'"
...
def main():
    ...
    path = build_resource_path(module.params["vif_uuid"], module.params["subresource"])
    params = build_query_params(module)

    try:
        client = new_xoa_client(module)
        response, status_code = client.get("vifs", path, params=params)
