...
DOCUMENTATION = r"""
---
module: xoa_task_info
short_description: Gather information about Xen Orchestra tasks
description:
  - Gather information about tasks through the Xen Orchestra REST API.
  - When O(task_uuid) is omitted, the module queries the task collection endpoint and can
    filter, limit, and select fields from the returned task list.
  - When O(task_uuid) is provided, the module returns detailed information for the task
    identified by O(task_uuid).
version_added: "1.0.0"
author:
  - w0
options:
  ...
  task_uuid:
    description:
      - UUID of the task to query.
      - When omitted, the module queries the task collection endpoint.
    type: str
  fields:
    description:
      - List of fields to request from the Xen Orchestra API.
      - Values are joined with commas before being sent to Xen Orchestra.
      - Supported for task collection queries.
      - Ignored for task detail queries.
    type: list
    elements: str
  filter:
    description:
...
notes:
  ...
  - This module maps to the Xen Orchestra C(/tasks), and C(/tasks/{id}) endpoints.
...
EXAMPLES = r"""
- name: List tasks with selected fields
  w0.xen_orchestra.xoa_task_info:
...
- name: Get a single task by UUID
  w0.xen_orchestra.xoa_task_info:
    api_host: xo.example.com
    username: admin
    password: secret
    task_uuid: 613f541c-4bed-fc77-7ca8-2db6b68f079c

- name: List tasks with token authentication
  w0.xen_orchestra.xoa_task_info:
    api_host: xo.example.com
    token: "{{ xo_token }}"
    filter:
...
result:
  description:
    ...
    - Task collection queries return a list of task records.
    - Task detail queries return a single task object.
  returned: success
  type: raw
changed:
  description:
...
def _validate_request_shape(module):
    ...
    task_uuid = module.params["task_uuid"]

    provided = provided_optional_params(module)

    if not task_uuid:
        allowed = STANDARD_COLLECTION_PARAMS
        label = "task collection request"
    else:
        allowed = set()
        label = "task detail request"
...
def main():
    ...
    path = build_resource_path(module.params["task_uuid"], module.params["subresource"])
    params = build_query_params(module)

    try:
        client = new_xoa_client(module)
        response, status_code = client.get("tasks", path, params=params)
