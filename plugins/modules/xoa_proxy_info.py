...
DOCUMENTATION = r"""
---
module: xoa_proxy_info
short_description: Gather information about Xen Orchestra proxies
description:
  - Gather information about proxies through the Xen Orchestra REST API.
  - When O(proxy_uuid) is omitted, the module queries the proxy collection endpoint and can
    filter, limit, and select fields from the returned proxy list.
  - When O(proxy_uuid) is provided, the module returns detailed information for the proxy
    identified by O(proxy_uuid).
version_added: "1.0.0"
author:
  - w0
options:
  ...
  proxy_uuid:
    description:
      - UUID of the proxy to query.
      - When omitted, the module queries the proxy collection endpoint.
    type: str
  fields:
    description:
      - List of fields to request from the Xen Orchestra API.
      - Values are joined with commas before being sent to Xen Orchestra.
      - Supported for proxy collection queries.
      - Ignored for proxy detail queries.
    type: list
    elements: str
  filter:
    description:
...
notes:
  ...
  - This module maps to the Xen Orchestra C(/proxies) and C(/proxies/{id}) endpoints.
...
EXAMPLES = r"""
- name: List proxies with selected fields
  w0.xen_orchestra.xoa_proxy_info:
...
- name: Get a single proxy by UUID
  w0.xen_orchestra.xoa_proxy_info:
    api_host: xo.example.com
    username: admin
    password: secret
    proxy_uuid: 613f541c-4bed-fc77-7ca8-2db6b68f079c

- name: List proxies with token authentication
  w0.xen_orchestra.xoa_proxy_info:
    api_host: xo.example.com
    token: "{{ xo_token }}"
    filter:
...
result:
  description:
    ...
    - Proxy collection queries return a list of proxy records.
    - Proxy detail queries return a single proxy object.
  returned: success
  type: raw
changed:
  description:
