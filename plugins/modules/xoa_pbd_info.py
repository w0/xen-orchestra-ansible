...
---
module: xoa_pbd_info
short_description: Gather information about Xen Orchestra PBDs
description:
  - Gather information about PBDs through the Xen Orchestra REST API.
  - When O(pbd_uuid) is omitted, the module queries the PBD collection endpoint and can
    filter, limit, and select fields from the returned PBD list.
  - When O(pbd_uuid) is provided, the module returns detailed information for the PBD
    identified by O(pbd_uuid).
version_added: "1.0.0"
author:
  - w0
options:
  ...
  pbd_uuid:
    description:
      - UUID of the PBD to query.
      - When omitted, the module queries the PBD collection endpoint.
    type: str
  fields:
    description:
      - List of fields to request from the Xen Orchestra API.
      - Values are joined with commas before being sent to Xen Orchestra.
      - Supported for PBD collection queries.
      - Ignored for PBD detail queries.
    type: list
    elements: str
  filter:
    description:
...
notes:
  ...
  - This module maps to the Xen Orchestra C(/pbds) and C(/pbds/{id}) endpoints.
...
EXAMPLES = r"""
- name: List PBDs with selected fields
  w0.xen_orchestra.xoa_pbd_info:
...
- name: Get a single PBD by UUID
  w0.xen_orchestra.xoa_pbd_info:
    api_host: xo.example.com
    username: admin
    password: secret
    pbd_uuid: 613f541c-4bed-fc77-7ca8-2db6b68f079c

- name: List PBDs with token authentication
  w0.xen_orchestra.xoa_pbd_info:
    api_host: xo.example.com
    token: "{{ xo_token }}"
    filter:
...
result:
  description:
    ...
    - PBD collection queries return a list of PBD records.
    - PBD detail queries return a single PBD object.
  returned: success
  type: raw
changed:
  description:
...
def _validate_request_shape(module):
    ...
    pbd_uuid = module.params["pbd_uuid"]

    provided = provided_optional_params(module)

    if not pbd_uuid:
        allowed = STANDARD_COLLECTION_PARAMS
        label = "pbd collection request"
    else:
        allowed = set()
        label = "pbd detail request"
...
def main():
    ...
    path = build_resource_path(module.params["pbd_uuid"], module.params["subresource"])
    params = build_query_params(module)

    try:
        client = new_xoa_client(module)
        response, status_code = client.get("pbds", path, params=params)
