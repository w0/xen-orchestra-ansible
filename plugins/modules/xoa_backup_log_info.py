...
DOCUMENTATION = r"""
---
module: xoa_backup_log_info
short_description: Gather information about Xen Orchestra backup logs
description:
  - Gather information about backup logs through the Xen Orchestra REST API.
  - When O(backup_log_uuid) is omitted, the module queries the backup log collection
    endpoint and can filter, limit, and select fields from the returned backup log list.
  - When O(backup_log_uuid) is provided, the module returns detailed information for the
    backup log identified by O(backup_log_uuid).
version_added: "1.0.0"
author:
  - w0
options:
  ...
  backup_log_uuid:
    description:
      - UUID of the backup log to query.
      - When omitted, the module queries the backup log collection endpoint.
    type: str
  fields:
    description:
      - List of fields to request from the Xen Orchestra API.
      - Values are joined with commas before being sent to Xen Orchestra.
      - Supported for backup log collection queries.
      - Ignored for backup log detail queries.
    type: list
    elements: str
  filter:
    description:
...
EXAMPLES = r"""
- name: List backup logs with selected fields
  w0.xen_orchestra.xoa_backup_log_info:
...
- name: Get a single backup log by UUID
  w0.xen_orchestra.xoa_backup_log_info:
    api_host: xo.example.com
    username: admin
    password: secret
    backup_log_uuid: 613f541c-4bed-fc77-7ca8-2db6b68f079c

- name: List backup logs with token authentication
  w0.xen_orchestra.xoa_backup_log_info:
    api_host: xo.example.com
    token: "{{ xo_token }}"
    filter:
...
result:
  description:
    ...
    - Backup log collection queries return a list of backup log records.
    - Backup log detail queries return a single backup log object.
  returned: success
  type: raw
changed:
  description:
...
def _validate_request_shape(module):
    ...
    backup_log_uuid = module.params["backup_log_uuid"]

    provided = provided_optional_params(module)

    if not backup_log_uuid:
        allowed = STANDARD_COLLECTION_PARAMS
        label = "backup_log collection request"
    else:
        allowed = set()
        label = "backup_log detail request"
...
def main():
    ...
    path = build_resource_path(module.params["backup_log_uuid"], module.params["subresource"])
    params = build_query_params(module)

    try:
        client = new_xoa_client(module)
        response, status_code = client.get("backup-logs", path, params=params)
