...
DOCUMENTATION = r"""
---
module: xoa_backup_archive_info
short_description: Gather information about Xen Orchestra backup archives
description:
  - Gather information about backup archives through the Xen Orchestra REST API.
  - When O(backup_archive_uuid) is omitted, the module queries the backup archive collection
    endpoint and can filter, limit, and select fields from the returned backup archive list.
  - When O(backup_archive_uuid) is provided, the module returns detailed information for the
    backup archive identified by O(backup_archive_uuid).
version_added: "1.0.0"
author:
  - w0
options:
  ...
  backup_archive_uuid:
    description:
      - UUID of the backup archive to query.
      - When omitted, the module queries the backup archive collection endpoint.
    type: str
  fields:
    description:
      - List of fields to request from the Xen Orchestra API.
      - Values are joined with commas before being sent to Xen Orchestra.
      - Supported for backup archive collection queries.
      - Ignored for backup archive detail queries.
    type: list
    elements: str
  filter:
    description:
...
notes:
  ...
  - This module maps to the Xen Orchestra C(/backup-archives), and
    C(/backup-archives/{id}) endpoints.
...
EXAMPLES = r"""
- name: List backup archives with selected fields
  w0.xen_orchestra.xoa_backup_archive_info:
...
- name: Get a single backup archive by UUID
  w0.xen_orchestra.xoa_backup_archive_info:
    api_host: xo.example.com
    username: admin
    password: secret
    backup_archive_uuid: 613f541c-4bed-fc77-7ca8-2db6b68f079c

- name: List backup archives with token authentication
  w0.xen_orchestra.xoa_backup_archive_info:
    api_host: xo.example.com
    token: "{{ xo_token }}"
    filter:
...
result:
  description:
    ...
    - Backup archive collection queries return a list of backup archive records.
    - Backup archive detail queries return a single backup archive object.
  returned: success
  type: raw
changed:
  description:
...
def _validate_request_shape(module):
    ...
    backup_archive_uuid = module.params["backup_archive_uuid"]

    provided = provided_optional_params(module)

    if not backup_archive_uuid:
        allowed = STANDARD_COLLECTION_PARAMS
        label = "backup_archive collection request"
    else:
        allowed = set()
        label = "backup_archive detail request"
...
def main():
    module = AnsibleModule(
        argument_spec=build_xoa_argument_spec(
            backup_archive_uuid=dict(type="str"),
    ...
    path = build_resource_path(module.params["backup_archive_uuid"], module.params["subresource"])
    params = build_query_params(module)

    try:
        client = new_xoa_client(module)
        response, status_code = client.get("backup-archives", path, params=params)
