...
---
module: xoa_sr_info
short_description: Gather information about Xen Orchestra storage repositories
description:
  - Gather information about storage repositories through the Xen Orchestra REST API.
  - When O(sr_uuid) is omitted, the module queries the storage repository collection endpoint and can
    filter, limit, and select fields from the returned storage repository list.
  - When O(sr_uuid) is provided without O(subresource), the module returns detailed
    information for the storage repository identified by O(sr_uuid).
  - When both O(sr_uuid) and O(subresource) are provided, the module returns the
    requested subresource for that storage repository.
...
options:
  ...
  sr_uuid:
    description:
      - UUID of the storage repository to query.
      - When omitted, the module queries the storage repository collection endpoint.
      - Required when O(subresource) is specified.
    type: str
  subresource:
    description:
      - Storage repository subresource to query for the storage repository identified by
        O(sr_uuid).
  ...
  fields:
    description:
      ...
      - Supported for storage repository collection queries and for the O(alarms),
        O(messages), and O(tasks) subresources.
      - Ignored for storage repository detail queries.
    type: list
    elements: str
  filter:
    description:
...
notes:
  ...
  - This module maps to the Xen Orchestra C(/srs), C(/srs/{id}), and selected
    C(/srs/{id}/{subresource}) endpoints.
...
EXAMPLES = r"""
- name: List storage repositories with selected fields
  w0.xen_orchestra.xoa_sr_info:
...
- name: Get a single storage repository by UUID
  w0.xen_orchestra.xoa_sr_info:
    api_host: xo.example.com
    username: admin
    password: secret
    sr_uuid: 613f541c-4bed-fc77-7ca8-2db6b68f079c

- name: Get storage repository messages with selected fields
  w0.xen_orchestra.xoa_sr_info:
    api_host: xo.example.com
    username: admin
    password: secret
    sr_uuid: cef5f68c-61ae-3831-d2e6-1590d4934acf
...
- name: Get storage repository alarms
  w0.xen_orchestra.xoa_sr_info:
    api_host: xo.example.com
    username: admin
    password: secret
    sr_uuid: f07ab729-c0e8-721c-45ec-f11276377030
    subresource: alarms
    fields:
      - id
      - time

- name: Get storage repository tasks with token authentication
  w0.xen_orchestra.xoa_sr_info:
    api_host: xo.example.com
    token: "{{ xo_token }}"
    sr_uuid: 613f541c-4bed-fc77-7ca8-2db6b68f079c
...
result:
  description:
    ...
    - Storage repository collection queries return a list of storage repository records.
    - Storage repository detail queries return a single storage repository object.
