...
DOCUMENTATION = r"""
---
module: xoa_message_info
short_description: Gather information about Xen Orchestra messages
description:
  - Gather information about messages through the Xen Orchestra REST API.
  - When O(message_uuid) is omitted, the module queries the message collection endpoint and can
    filter, limit, and select fields from the returned message list.
  - When O(message_uuid) is provided, the module returns detailed information for the message
    identified by O(message_uuid).
version_added: "1.0.0"
author:
  - w0
options:
  ...
  message_uuid:
    description:
      - UUID of the message to query.
      - When omitted, the module queries the message collection endpoint.
    type: str
  fields:
    description:
      - List of fields to request from the Xen Orchestra API.
      - Values are joined with commas before being sent to Xen Orchestra.
      - Supported for message collection queries.
      - Ignored for message detail queries.
    type: list
    elements: str
  filter:
    description:
...
notes:
  ...
  - This module maps to the Xen Orchestra C(/messages), and C(/messages/{id}) endpoints.
...
EXAMPLES = r"""
- name: List messages with selected fields
  w0.xen_orchestra.xoa_message_info:
...
- name: Get a single message by UUID
  w0.xen_orchestra.xoa_message_info:
    api_host: xo.example.com
    username: admin
    password: secret
    message_uuid: 613f541c-4bed-fc77-7ca8-2db6b68f079c

- name: List messages with token authentication
  w0.xen_orchestra.xoa_message_info:
    api_host: xo.example.com
    token: "{{ xo_token }}"
    filter:
...
result:
  description:
    ...
    - Message collection queries return a list of message records.
    - Message detail queries return a single message object.
  returned: success
  type: raw
changed:
  description:
